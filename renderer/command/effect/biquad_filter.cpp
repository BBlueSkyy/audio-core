...
/**
 * Native float-coefficient biquad used by Audio Renderer REV15+.
 */
void ApplyBiquadFilterFloat2(std::span<s32> output, std::span<const s32> input,
    ...
    for (u32 i = 0; i < sample_count; i++) {
        ...
        const f64 sample{
            in_sample * static_cast<f64>(b[0]) +
            s[0] * static_cast<f64>(b[1]) +
            s[1] * static_cast<f64>(b[2]) +
            s[2] * static_cast<f64>(a[0]) +
            s[3] * static_cast<f64>(a[1])};
